function ya_plotAllProbeTracksInROIs(allenAtlasPath, atlasBrainRegLocation, paths, regionNames, regionColors, regionPlotSide, plotPatch,...
...
if nargin < 5 || isempty(regionColors)
    regionColors = ya_getColors(length(regionNames));
end
...
if ~isempty(regionNames)
    ...
    probeColors = ya_getColors(1000); % some large number
    if ~isempty(paths)
        for iAnimal = 1:length(paths, 2)
