...
for iProbe = 1:gui_data.n_probes
    ...
    %gui_data.probe_points{iProbe} = plot(NaN, NaN, 'ro', 'ButtonDownFcn', @(gui_fig)startDragFcn(gui_fig), 'Tag', 'draggable');%gobjects(10, 1);
    %gui_data.hPointsPlot = plot(NaN, NaN, 'ro', 'ButtonDownFcn', @startDragFcn, 'Tag', 'draggable'); % initialize an empty plot for control points
end
...
% hold on;
% for iProbe = 1:gui_data.n_probes
%     gui_data.hPointsPlot(iProbe) = plot(NaN, NaN, 'ro', 'ButtonDownFcn', @startDragFcn, 'Tag', 'draggable'); % initialize an empty plot for control points
%     gui_data.hCurvePlot(iProbe) = plot(NaN, NaN, 'r', 'LineWidth', 2); % initialize an empty plot for Bezier curve
% end
% gui_data.hHighlighted = scatter(NaN, NaN, 'b*', 'ButtonDownFcn', @startDragFcn, 'Tag', 'draggable');
% hold off;
% gui_data.hListbox = uicontrol('Style', 'listbox', 'Position', [650, 250, 500, 300], 'String', {}, 'Min', 0, 'Max', 2, 'Callback', @(src, event)highlightSelectedPoint());
% gui_data.deletePointButton = uicontrol('Style', 'pushbutton', 'Position', [650, 210, 130, 30], 'String', 'Delete Point', 'Callback', @(src, event)deleteSelectedPoint());
% Initialize Bezier curve points and lines
gui_data.bezier_control_points = cell(gui_data.n_probes, 1);
gui_data.bezier_curves = gobjects(gui_data.n_probes, 1);
% gui_data.points_table = uitable('Parent', gui_fig, ...
